...
vector n a 
    ...
    |otherwise = V (2^n) (gen n a)

gen::Int -> a ->Tree a
gen 0 a = Leaf a
gen n a = Node (gen (n-1) a) (gen (n-1) a)
...
get a (V n (Leaf c)) = c
get a (V n (Node l r))
  |a<0 = error "numero negativo"
  |a>=n = error "el valor dado es mayor que el indice del arbol"
  |odd a = get (a `div` 2) (V (n `div` 2) r)
  |otherwise = get (a `div` 2) (V (n `div` 2) l)
...
set 1 x(V n (Leaf c)) = V 1 (Leaf x)
set a x(V n (Node l r))
  |a<0 = error "numero negativo"
  |a>=n = error "el valor dado es mayor que el indice del arbol"
  |odd a = V n (Node l (setE (a `div` 2) x r))
  |otherwise =V n (Node (setE (a `div` 2) x l) r)

setE::Int->a->Tree a->Tree a
setE a x (Leaf c) = Leaf x
setE a x (Node l r)
  |even a= Node (setE (a `div` 2) x l) r
  |otherwise= Node l (setE (a `div` 2) x r)
