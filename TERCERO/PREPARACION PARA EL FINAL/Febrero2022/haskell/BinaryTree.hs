...
greaterSumTree bst = fst (greaterSumaux bst 0)
  where
    greaterSumaux Empty acum = (Empty, acum);
    greaterSumaux (Node x l r) acum = ((Node acumr l' r'), acuml)
      where
        (r', acumr) = greaterSumaux r acum
        (l', acuml) = greaterSumaux l (acumr+x)
