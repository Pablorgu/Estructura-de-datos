...
fromJust:: Maybe a ->a
fromJust Nothing = error "cositas"
fromJust (Just a) = a

-- | Exercise 1

weights :: Ord a => [a] -> D.Dictionary a Int
weights a = aux a D.empty
  where 
    aux [] d = d
    aux (x:xs) d
...
huffmanLeaves s = aux (nub s) (weights s) (PQ.empty)
  where
    aux [] d pq = pq
    aux (x:xs) d pq = aux xs d (PQ.enqueue (WLeaf x (fromJust(D.valueOf x d))) pq)
...
huffmanTree x = if length (nub x) < 2 then error "There aren't enough symbols" else aux (PQ.first (huffmanLeaves x)) (PQ.dequeue (huffmanLeaves x))
  where
    aux n pq
      | PQ.isEmpty pq = n
      | otherwise = aux (merge n (PQ.first pq)) (PQ.dequeue pq)
...
joinDics d1 d2 = aux d1 (D.keysAndValues d2)
  where
    aux d1 [] = d1
    aux d1 (x:xs) = aux (D.insert k v d1) xs
      where (k, v) = x
...
prefixWith x dic = aux (zip (D.keys dic) (map (x:) (D.values dic))) D.empty
  where 
    aux [] d = d
    aux (x:xs) d = aux xs (D.insert k v d)
      where
        (k, v) = x
...
huffmanCode (WNode l r _) = joinDics (prefixWith 1 (huffmanCode l)) (prefixWith 0 (huffmanCode r))
