...
insert x (Node y oy b)
   ...
   |x == y = Node y (oy + 1) b
   |otherwise = insert y oy (insert x b)
...
isEmpty_insert x b = isEmpty (insert x b)

occurrences_empty X = ocurrences x empty == 0
occurrences_insert_1 x y b = x == y ==> ocurrences x (insert y b) == 1 + ocurrences x b
occurrences_insert_2 x y b = x /= y ==> delete x (insert y b) == insert (delete x b)

-- transformadores
delete_empty = delete x empty == empty
delete_insert_1 = x y b = x == y ==> delete x (insert y b) == b
