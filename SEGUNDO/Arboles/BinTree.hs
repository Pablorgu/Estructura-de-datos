...
atLevelB i (NodeB _ l r) = atLevelB )(i-1) l ++ atLevelB (i-1) r
atLevelB _ _
...
pathsToB _ EmptyB = []
pathsToB e (Nodeb x l r)
  |e == x = [[x]]
  |otherwise = map(x:)(pathsToB e | ++ pathsToB e r)
-- | caminos exhaustivos hasta el nodo 'x' en un árbol binario
-- >>>
pathsToB2 :: (Eq a) => a -> TreeB a -> [[a]]
pathsToB2 _ EmptyB = []
pathsToB2 x (NodeB y l r)
  |x == y = [[x]] ++ map (x:) (pathsToB2 x l ++ pathsToB2 x r)
  |otherwise  = map (y:) (pathsToB2 x | ++pathsToB2 x r)

-- | recorrido inorden de un árbol binario
-- >>> inOrderB tree4
-- [4,2,5,1,6,3,7]
inOrderB :: TreeB a -> [a]
inOrderB EmptyB = undefined 
inOrderB (NodeB x l r = undefined )
