...
--
  | not (D.isDefinedAt k dict) =  aux g (D.insert k v dict) (stacknew (S.pop stack) (Graph.successors g k) (k,v))
  |otherwise = if v /= coloractual then Nothing else aux g dict (S.pop stack)
  where
    (k,v) = (S.top stack)
    Just coloractual = (D.valueOf k dict)
    stacknew s x t = pushAll s [(c, (nextColor (snd t))) | c <- x]
