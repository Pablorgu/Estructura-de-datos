...
bolsa2 :: Bag Char
bolsa2 = Node 'a' 1 (Node 'b' 1 (Node 'c' 1 (Node 'd'  1 Empty)))
...
insert x (Node y oy s) 
   ...
   |x>y = insert y (insert x s)
   |otherwise = (Node y (oy+1) s)
...
--------------------------------------------------------
-- selectores
isEmpty_empty = isEmpty empty == undefined
isEmpty_insert x s = isEmpty (insert x s) == undefined

occurrences_empty x = occurrences x empty == undefined
occurrences_insert_1 x s = occurrences x (insert x s) == undefined
occurrences_insert_2 x y s = x /= y ==> occurrences x (insert y s) == undefined

-- transformadores
delete_empty x = delete x empty == undefined
...
union (Node x ox s) (Node y oy t)
   ...
   |y<x = Node y oy (union (Node x ox s)  t)
   |otherwise = Node x (ox + oy) (union s t)
