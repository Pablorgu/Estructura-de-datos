...
union Emtpy s2 = s2
union s1 Empty = s1
union (Node x num1 s1)(Node y num2 s2)
   |x==y Node x num1 (union s1 s2)
   |x>y x Node x num1(union s1 (Node y num2 s2))
   |x>y x Node y num2(union (Node x num1 s1) s2)

-- |
-- >>> intersection b1 b2
-- LinearBag { 'a' }
intersection :: Ord a => Bag a -> Bag a -> Bag a
intersection Emtpy _ = Empty
