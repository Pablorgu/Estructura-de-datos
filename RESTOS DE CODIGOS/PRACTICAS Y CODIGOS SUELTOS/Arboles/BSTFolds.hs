...
sumBST x = foldInOrder (+) 0 x

-- |
-- >>> weightBST t1
-- 7
weightBST :: BST a -> Integer
weightBST x = foldInOrder (\ current next -> 1 + next) 0 x
...
evenBST x = [p | p <- (filter even nodes)]
  where
    nodes = (foldInOrder (:) [] x)
...
filterBST f x = foldInOrder (\ current sigNode -> if f current then current:sigNode else sigNode) [] x