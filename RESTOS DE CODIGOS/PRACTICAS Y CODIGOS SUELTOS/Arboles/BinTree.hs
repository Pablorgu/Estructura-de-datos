...
isElemB a (NodeB x l r)
  |a == x = True
  |otherwise = isElemB a l|| isElemB a r
...
atLevelB _ EmptyB = []
atLevelB 0 (NodeB x l r) = [x]
atLevelB n (NodeB x l r) = atLevelB (n-1) l ++ atLevelB (n-1) r
...
pathsToB _ EmptyB = []
pathsToB n (NodeB x l r)
  |x==n = [[x]]
  |otherwise= map (x:) (pathsToB n l ++ pathsToB n r)--cuando el map se aplica a una lista vacia devuelve la lista vacia

-- | caminos exhaustivos hasta el nodo 'x' en un árbol binario
-- >>>
pathsToB2 :: (Eq a) => a -> TreeB a -> [[a]]
pathsToB2 = undefined--suerte que ni diego la sabe
