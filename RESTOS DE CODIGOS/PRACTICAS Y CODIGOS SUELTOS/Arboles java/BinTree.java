...
public class BinTree<T> {
    ...
    public static <E> int weightRec (Tree<E> node) {
        if(node == null){
            return 0;
        } else {
    ...
    public static <E> int heightRec(Tree<E> node) {
        if(node == null){
            return 0;
        }else {
    ...
    public static <E> int borderRec(Tree<E> node) {
        if(node == null) {
            return 0;
        } else if(isLeaf(node)) {
            ...
            List<E> bleft = new LinkedList<>();
            List<E> bright = new LinkedList<>();
            for(E leaft:bright){
                bleft.append(leaf);
            }
            return bleft;
    ...
    public static <T,E> boolean iElemRec(T x, Tree<E> node){
        if(node == null) {
            return false;
        } else {
    ...
    private static <E> List<E> atLevelRec(int i, Tree<E> node){
        if(i==0 && node!=null) {
            ...
            List<E> leftlev = atLevelRec(i-1, node.left);
            List<E> rightlev = atLevelRec(i-1, node.right);
            for(E n:rightlev){
                leftlev.append(n);
            }
            return leftlev;
    ...
    public static <E> int inOrderRec(Tree<E> node) {
        Tree<E> father=node;
        if(node==null) {
            return new LinkedList<>();
        } else {
            List<E> left = inOrderRec(node);
            List<E> right = inOrderRec(node);
            left.append(father.elem);
            for(E n:right){
                left.append(n);
            }
            return left;
